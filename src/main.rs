...
fn read_hdf5(filename: &str, temperature_key: &str) -> HashMap<String, HashMap<String, Vec<f64>>> {
    // Open the HDF5 file
    ...
    for reaction_name in group_reactions_names {
        ...
        reactions_map.insert(reaction_name, data);
    }

    let mut energy_map = HashMap::new();
    energy_map.insert(temperature_key.to_string(), energy_data);
...
fn main() {
    ...
    const REACTION_MT: &str = "reaction_444";

    // Call the read_hdf5 function with the filename and dataset key as arguments
    let result = read_hdf5(FILENAME, TEMPERATURE);

    // Print the energy values
    println!("Energy dataset: {:?}", result.get("energy").unwrap());
    
    
    let reactions = result.get("reactions").unwrap().get(REACTION_MT);
        println!("MT reaction: {:?}", reactions);
    }
